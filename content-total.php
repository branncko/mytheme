	...
	<figure class="mh-posts-large-thumb">
		<a class="mh-thumb-icon" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
			if (has_post_thumbnail()) {
				...
				if ($mh_magazine_options['sidebars'] === 'disable') {
					echo '<img class="mh-image-placeholder" src="' . get_template_directory_uri() . '/images/placeholder-slider.png' . '" alt="' . esc_html__('No Picture', 'prefeitura') . '" />';
				} else {
					echo '<img class="mh-image-placeholder" src="' . get_template_directory_uri() . '/images/placeholder-content.png' . '" alt="' . esc_html__('No Picture', 'prefeitura') . '" />';
				}
			} ?>
		</a>
		<?php if (has_category()) { ?>
